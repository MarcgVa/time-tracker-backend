...
const createInvoice = async (req, res, next) => {
  try {
    ...
    const seconds = project.timeEntries.reduce(
      (s, e) =>
        s +
        (e.endTime && e.startTime
          ? (new Date(e.endTime) - new Date(e.startTime)) / 1000
          : 0),
      0
    );
    ...
    if (!invoice)
      return res.status(404).json({ error: "Failed to create invoice" });
...
const delInvoice = async (req, res, next) => {
  try {
    ...
    if (!invoice)
      return res.status(404).json({ error: "Unable to delete invoice" });
    res.Status(204);
  } catch (err) {
    console.error(err);
  }
